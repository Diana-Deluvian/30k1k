    ...
    for (const property in book) {
      if (Array.isArray(book[property])) {
        ...
        book[property].forEach((elem) => {
          if (elem.value.toLowerCase().includes(searchTerm.toLowerCase())) {
            isFound = true;
          }
        });
    ...
    <React.Fragment>
      <div className='book-list'>
        {!_.isEmpty(auth) ? (
          ...
          showErr && (
            <Alert
              style={{ maxWidth: '60ch', margin: 'auto' }}
              variant='danger'
              onClose={() =>
                setAuth((prevState) => ({ ...prevState, showErr: false }))
              }
              dismissible
            >
        ...
        {!_.isEmpty(books) ? (
          ...
          <p className='message'>Books are loading, please wait.</p>
        )}
      </div>
    </React.Fragment>
  );
